...
Scripts.prototype.lookupRelative = function (file, target) {
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    for (var j = 0; j < extensions.length; j++) {
      ...
      if (f.path === path + extensions[j]) return name;
    }
    // check by removing extensions
    if (f.path.replace(/\.\w+$/, '') === path) return name;
  }
...
Scripts.prototype.lookupDependency = function* (file, target) {
  ...
  // it is safe to translate reference to lowercase here
  // but the rest should be kept untouched
